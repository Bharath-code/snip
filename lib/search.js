 ...
 * Supports content-aware search — snippet content is indexed
 * alongside names and tags so "find my docker cleanup command"
 * matches snippets containing those words.
 *
 * @module search
 */
...
function _getDbMtime() {
  try {
    ...
    const dbPath = useSqlite(cfg) ? (cfg.sqlitePath || cfg.dbPath) : cfg.dbPath;
...
/**
 * Build a fresh Fuse index.
 * Indexes name, content, and tags with decreasing weights:
 *   name (0.5) — exact snippet name match
 *   content (0.3) — words inside the snippet body
 *   tags (0.2) — organization tags
 *
 * Content is loaded via storage.readSnippetContent(), which handles
 * both JSON (read from file) and SQLite (fetch from column) backends.
 *
...
function _buildFreshIndex(list) {
  const items = list.map(s => ({
    ...
    name: s.name,
    tags: (s.tags || []).join(' '),
    content: (() => { try { return storage.readSnippetContent(s) || ''; } catch { return ''; } })().slice(0, 10000) // limit to first 10KB
  }));
...
/**
 * Search snippets and return results with Fuse relevance scores.
 * A lower score means a better match (0 = perfect match, 1 = no match).
 * Content-aware — matches against name, content body, and tags.
 *
 * @param {string} query - Search query
 * @param {number} [limit=15] - Maximum results to return
 * @returns {Array<{item: {id: string, name: string, tags: string, content: string}, score: number}>}
 */
function searchWithScores(query, limit = 15) {
  if (!query || !String(query).trim()) {
    return [];
  }
...
module.exports = { search, searchWithScores, suggestSimilar, invalidateCache };
