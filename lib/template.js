 ...
 * Returns Promise resolving to content string. Throws on abort (no process.exit).
 */
async function promptAndInterpolate(content) {
    ...
    for (const v of vars) {
        ...
        if (!answer && v.defaultValue === null) {
            ...
            if (!answer) {
                throw new Error('Aborted: required variable not provided.');
            }
        }
