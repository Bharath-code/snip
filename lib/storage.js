...
function addSnippet({ name, content, language, tags }) {
  ...
  if (useSqlite(cfg)) {
    ...
    return { id, name: safeName, content, language: language || '', tags: safeTags, createdAt: now, updatedAt: now, path: null };
  }
...
function getSnippetByIdOrName(idOrName) {
  ...
  if (useSqlite(cfg)) {
    ...
    if (row) return { id: row.id, name: row.name, language: row.language, tags: JSON.parse(row.tags || '[]'), createdAt: row.createdAt, updatedAt: row.updatedAt, lastUsedAt: row.lastUsedAt, usageCount: row.usageCount || 0, path: null };
...
process.on('SIGINT', () => { _forceSave(); process.exit(130); });
process.on('SIGTERM', () => { _forceSave(); process.exit(143); });
