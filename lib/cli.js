...
program
  .command('init')
  .description('Scaffold a repo runbook: .snip/ with snippets.json + policy.json + README')
  .option('--workspace <name>', 'Workspace name (default: directory name)')
...
if (!isQuiet() && process.argv.length <= 2) {
  try {
    if (!fs.existsSync(cfg.dbPath)) {
      ...
      console.log('');
      console.log(c.muted('  Tip: Run ') + c.brand('snip init') + c.muted(' in a repo to scaffold a .snip/ runbook for your team + AI agents'));
      console.log('');
    }
  } catch (_e) { }
}
