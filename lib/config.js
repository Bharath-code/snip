...
function loadConfig() {
  ...
  try {
    if (fs.existsSync(CONFIG_FILE)) {
      ...
      const allowedKeys = ['editor', 'dataDir', 'dbPath', 'useSqlite', 'sqlitePath', 'defaultShell', 'confirmRun', 'gist_token', 'sortMode', 'ai_provider', 'ai_api_key', 'ai_model', 'ai_max_tokens', 'teamDir'];
...
/**
 * Check if SQLite backend is enabled for storage.
 * Detects both the explicit `useSqlite` flag and implicit `.sqlite`/`.db` file extensions.
...
module.exports = { ensureDirs, loadConfig, saveConfig, useSqlite, CONFIG_FILE, DB_FILE };
