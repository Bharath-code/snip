const dangerousPatterns = [
  ...
  /:\s*\(\s*\)\s*\{[^}]*\|[^}]*&[^}]*\}\s*;\s*:/i,  // fork bomb pattern (e.g. :(){ :|:& };:)
 ...
 * Always prompts the user explicitly. Returns Promise<boolean>.
 */
async function confirmDangerous(content) {
  ...
  console.error('');
  const ans = await question('  Type "yes" to confirm execution (anything else aborts): ');
