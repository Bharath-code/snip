...
const { exitProcess } = require('./cli-utils');
...
if (!process._snipExecHandlers) {
  process._snipExecHandlers = true;
  process.once('SIGINT', () => { _cleanupTmp(); exitProcess(130); });
  process.once('SIGTERM', () => { _cleanupTmp(); exitProcess(143); });
}
...
function runSnippetContent(content, opts = {}) {
  ...
  try {
    const spawnOpts = opts.stdinData
      ? { input: opts.stdinData, stdio: ['pipe', 'inherit', 'inherit'] }
      : { stdio: 'inherit' };
    if (opts.timeout) spawnOpts.timeout = opts.timeout;
    const res = run(runner.command, [file], spawnOpts);
    if (res.error && res.error.code === 'ETIMEDOUT') {
      console.error(`Snippet killed: exceeded max runtime (${opts.timeout}ms)`);
      return 124;
    }
