...
const TOOLS = [
  ...
  {
    name: 'snip_exec',
    description: 'Execute a snippet by name. Dry-run mode is default — set dry_run: false to actually run it. Only shell/bash snippets can be executed for safety.',
    inputSchema: {
      type: 'object',
      properties: {
    ...
    return {
      content: [{
        ...
        text: JSON.stringify({
          ...
          snippet: snippet.name,
          content,
          message: 'This snippet contains potentially destructive operations. Use the CLI directly to confirm execution.',
        }, null, 2),
      }],
      isError: true,
    };
