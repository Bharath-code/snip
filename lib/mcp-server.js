...
// ── Team snippets (repo runbook, .snip/snippets.json found from cwd) ──
// Read fresh on every call: the file is tiny and may change under git.

function teamEntries() {
  const data = team.readTeamFile();
  if (!data) return [];
...
function findTeamSnippet(name) {
  return teamEntries().find(s => s.name === name) || null;
}

function searchTeam(query, limit) {
...
const TOOLS = [
  {
    name: 'snip_search',
    description: 'Fuzzy-search snippets by name, tags, or content. Team runbook snippets (.snip/ in the repo) rank first; each result is tagged source: team|personal with author/updatedAt provenance.',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  {
    name: 'snip_list',
    description: 'List all snippets with optional filters for tag, language, and sorting. Team runbook snippets come first, tagged source: team|personal.',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  {
    name: 'snip_read',
    description: "Read a snippet's full content and metadata by name or ID. Team runbook snippets are checked first and include author/updatedAt provenance.",
    inputSchema: {
      type: 'object',
      properties: {
  ...
  {
    name: 'snip_exec',
    description: 'Execute a snippet by name (team runbook snippets are resolved first). Dry-run mode is default — set dry_run: false to actually run it. Execution is governed by .snip/policy.json (deny/allow patterns, language allowlist, max runtime) plus built-in safety rules; policies may require human approval via `snip approve <id>`. Every call is written to the audit log.',
    inputSchema: {
      type: 'object',
      properties: {
...
function handleSearch(args) {
  ...
  // Team runbook matches come first; team shadows personal on name clashes.
  const teamHits = searchTeam(query, limit).map(r => ({
    name: r.item.name,
  ...
  const teamNames = new Set(teamHits.map(s => s.name));

  const results = search.search(query, limit);
  const personal = results.map(r => {
  ...
  }).filter(s => !teamNames.has(s.name));

  const snippets = [...teamHits, ...personal].slice(0, limit);
...
function handleSearchRelevance(args) {
  ...
  const teamHits = searchTeam(query, limit)
    .filter(r => minScore === undefined || isNaN(minScore) || r.score <= minScore)
    .map(r => ({
      name: r.item.name,
      score: r.score,
      tags: r.item.tags,
  ...
  const results = [...teamHits, ...personal].slice(0, limit);
...
function handleList(args) {
  ...
  // Team runbook snippets come first; team shadows personal on name clashes.
  let teamList = teamEntries();
  if (args.tag) teamList = teamList.filter(s => s.tags.includes(args.tag));
  if (args.lang) teamList = teamList.filter(s => s.language.toLowerCase() === args.lang.toLowerCase());
  teamList.sort((a, b) => a.name.localeCompare(b.name));
  const teamNames = new Set(teamList.map(s => s.name));
  ...
  const merged = [
    ...
    ...snippets.filter(s => !teamNames.has(s.name)).map(s => ({
      id: s.id,
      name: s.name,
      language: s.language,
      tags: s.tags,
...
function handleRead(args) {
  ...
  const teamSnip = findTeamSnippet(name);
  if (teamSnip) {
    return {
      content: [{
        type: 'text',
        text: JSON.stringify({
          ...
          author: teamSnip.author,
          updatedAt: teamSnip.updatedAt,
          workspace: teamSnip.workspace,
          source: 'team',
        }, null, 2),
      }],
    };
...
function handleExec(args) {
  ...
  const teamSnip = findTeamSnippet(name);
  const snippet = teamSnip || storage.getSnippetByIdOrName(name);
  const source = teamSnip ? 'team' : 'personal';
  if (!snippet) {
    return {
  ...
  const content = teamSnip ? teamSnip.content : storage.readSnippetContent(snippet);
  ...
  if (dryRun) {
    ...
    return {
      _audit: { dryRun: true, blocked: check.blocked, source },
      content: [{
        type: 'text',
        text: JSON.stringify({
          snippet: snippet.name,
  ...
  if (check.blocked) {
    return {
      _audit: { dryRun: false, blocked: true, reason: check.reason, source },
      content: [{
        type: 'text',
        text: JSON.stringify({
  ...
  if (check.requiresApproval) {
    ...
    return {
      _audit: { dryRun: false, pendingApproval: pending.id, source },
      content: [{
        type: 'text',
        text: JSON.stringify({
  ...
  return {
    _audit: { dryRun: false, blocked: false, exitCode: status, source },
    content: [{
      type: 'text',
      text: JSON.stringify({
        snippet: snippet.name,
...
module.exports = {
  start, createServer, TOOLS,
  // exposed for tests
  _handlers: { handleSearch, handleSearchRelevance, handleList, handleRead, handleExec },
};
