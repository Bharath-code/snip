...
function list(opts) {
  ...
  if (filtered.length === 0) {
    ...
    console.log(c.dim('  Run snip seed for example snippets, or snip doctor to check your setup.'));
    return;
  }
  ...
  if (filtered.length >= 50 && process.stdout.isTTY) {
    console.log(c.dim('  Tip: use snip ui to browse, or --limit <n> to cap results'));
  }
}
