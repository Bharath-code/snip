...
function fzfSearch() {
    ...
    if (check.status !== 0) {
        ...
        setExitCode(1); // operational error: missing dependency
        return;
    }

    const all = storage.listSnippets();
    if (!all.length) {
        log('No snippets. Add one with: snip add <name> --lang sh --tags demo');
        return; // not an error, just nothing to show
    }
