...
function start() {
  ...
  function editSelected() {
    ...
    if (!snippet) return;
    const cfg = config.loadConfig();
    const editor = (cfg.editor || process.env.EDITOR || 'vi').split(' ');
    const content = storage.readSnippetContent(snippet);
    const tmpFile = path.join(os.tmpdir(), `snip-edit-${snippet.id}.tmp`);
    try {
      fs.writeFileSync(tmpFile, content, 'utf8');
      if (screen.leave) screen.leave();
      spawnSync(editor[0], editor.slice(1).concat([tmpFile]), { stdio: 'inherit' });
      if (screen.enter) screen.enter();
      ...
      flashHelp(` ${G.cross} Edit failed: ${err.message} `, THEME.error, 2600);
    } finally {
      try { fs.unlinkSync(tmpFile); } catch (e) { }
      listBox.focus();
      screen.render();
  ...
  function addSnippet() {
    ...
    const cfg = config.loadConfig();
    const editor = (cfg.editor || process.env.EDITOR || 'vi').split(' ');
    const tmpFile = path.join(os.tmpdir(), `snip-new-${Date.now()}.sh`);
    const template = '# New snippet — write your code below\n\n';
    try {
      ...
      if (content.trim() && content.trim() !== template.trim()) {
        // Derive name from first line comment or filename
        const firstLine = content.split('\n')[0] || '';
        let name = firstLine.replace(/^[#\/\/ ]+/, '').trim().slice(0, 40) || `snippet-${Date.now()}`;
        name = name.replace(/[^a-zA-Z0-9_-]/g, '-').toLowerCase();
        storage.addSnippet({ name, content, language: 'sh', tags: [] });
        flashHelp(` ${G.check} Added "${name}" — edit tags via CLI: snip edit ${name} `, THEME.success, 2600);
        reloadAll();
      } else {
        flashHelp(` Cancelled — empty snippet. `, THEME.muted, 1200);
      }
    } catch (err) {
      flashHelp(` ${G.cross} Add failed: ${err.message} `, THEME.error, 2600);
    } finally {
      try { fs.unlinkSync(tmpFile); } catch (e) { }
      listBox.focus();
      screen.render();
