...
let _highlight;
let _highlightSupports;
try {
  const hl = require('cli-highlight');
  _highlight = hl.highlight;
  _highlightSupports = hl.supportsLanguage;
} catch (_) {
  _highlight = null;
  _highlightSupports = null;
}
...
// Safe syntax highlighting - validates language before calling cli-highlight
function safeHighlight(code, language) {
  if (!_highlight || !language) return code;
  // Validate language is supported to avoid stderr spam
  if (_highlightSupports && !_highlightSupports(language)) return code;
  try {
    return _highlight(code, { language, ignoreIllegals: true });
  } catch (_) {
    return code;
  }
}
...
function _spawnExternal(screen, cmd, args) {
  try {
    screen.program.normalBuffer();
    screen.program.showCursor();
  } catch (_e) { /* older blessed versions */ }
...
function start() {
  ...
  // First-run hint (3.7)
  const dataDir = config.loadConfig().dataDir || path.join(os.homedir(), '.local', 'share', 'snip');
  const firstRunPath = path.join(dataDir, '.snip-ui-seen');
  if (!fs.existsSync(firstRunPath)) {
    try { fs.mkdirSync(path.dirname(firstRunPath), { recursive: true }); fs.writeFileSync(firstRunPath, '1', 'utf8'); } catch (_) {}
    setTimeout(() => {
      helpBar.style.fg = THEME.accent.fg;
      helpBar.setContent(` First time? j/k move  Enter open  r run  ? help `);
      screen.render();
      setTimeout(() => { helpBar.setContent(buildHelpBar()); helpBar.style.fg = THEME.footerFg; screen.render(); }, 5000);
    }, 400);
  }
  ...
  function updatePreview() {
    ...
    let display = content || '';
    if (display && snippet.language) {
      display = safeHighlight(display, snippet.language);
    }
