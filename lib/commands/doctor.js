...
function doctorCmd() {
    ...
    // 7. Widget (Ctrl+G) — check if installed in shell rc file
    const widgetRcPath = getWidgetRcPath(shellName);
    let widgetInstalled = false;
    let widgetDetail;
    const widgetInstallCmd = shellName.includes('fish')
      ? 'snip widget fish | source'
      : `eval "$(snip widget ${shellName.includes('zsh') ? 'zsh' : 'bash'})"`;
    if (widgetRcPath) {
      try {
        const rcContent = fs.readFileSync(widgetRcPath, 'utf8');
        widgetInstalled = /snip[-_]widget|__snip_widget|snip\s+widget/.test(rcContent);
      } catch (_) {
        // rc file doesn't exist yet — that's fine
      }
    }
    if (widgetInstalled) {
      widgetDetail = `installed in ${widgetRcPath}`;
    } else if (widgetRcPath) {
      widgetDetail = `not installed — add to ${widgetRcPath}: ${widgetInstallCmd}`;
    } else {
      widgetDetail = `unknown shell — add to your shell rc: ${widgetInstallCmd}`;
    }
    out.push({ label: 'Widget (Ctrl+G)', ok: widgetInstalled || null, detail: widgetDetail });

    // Helper: determine shell rc file path
    function getWidgetRcPath(shell) {
      const home = process.env.HOME || os.homedir();
      if (shell.includes('fish')) return path.join(home, '.config', 'fish', 'config.fish');
      if (shell.includes('zsh')) return path.join(home, '.zshrc');
      if (shell.includes('bash')) {
        // Prefer .bashrc (Linux) or .bash_profile (macOS) if .bashrc doesn't exist
    ...
    for (const c of out) {
        ...
        if (c.ok === false) hasErrors = true;
    }
    log('');

    if (hasErrors) setExitCode(1);
}
