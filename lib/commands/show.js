...
function show(idOrName, opts) {
  ...
  if (!s) {
    ...
    if (suggestions.length) {
      console.error(c.dim(`  Did you mean:`) + ' ' + suggestions.map(s => c.accent(s)).join(', ') + '?');
      console.error(c.dim(`  Or try:`) + c.dim(` snip search "${idOrName}"`));
    } else {
      console.error(c.dim(`  Run`) + c.dim(` snip list`) + c.dim(` to see all snippets`));
      console.error(c.dim(`  Or`) + c.dim(` snip search "${idOrName}"`) + c.dim(` to find similar`));
    }
    process.exitCode = 1;
    return;
  }
