...
async function watchCmd(idOrName, opts = {}) {
  ...
  function stop() {
    if (stopped) return;
    stopped = true;
    try { fs.unwatchFile(tmpFile); } catch (_) {}
  ...
  fs.watchFile(tmpFile, { interval: 100 }, (curr, prev) => {
    if (stopped) return;
    if (curr.mtimeMs === prev.mtimeMs) return; // Only trigger if modification time changed
    ...
    debounceTimer = setTimeout(() => {
      ...
      console.log('');
    }, 100); // 100ms is enough debounce for fs.watchFile
  });
