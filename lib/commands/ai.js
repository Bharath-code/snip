...
async function generate(prompt, opts) {
  if (!prompt || prompt.trim().length === 0) {
    ...
    log(c.dim('  Usage: snip ai generate "<prompt>"'));
    setExitCode(2); // misuse: missing prompt
    return;
  }
  ...
  if (!apiKey) {
    ...
    log(c.dim('  Set SNIP_AI_API_KEY environment variable or run:'));
    log(c.code('  snip config set ai_api_key <your-key>'));
    setExitCode(1);
    return;
  }
  ...
  try {
    ...
    log('');
    log(c.success('  ✓ Generated: ') + c.brand(snippet.name));
    log('');
    ...
    if (metaParts.length) {
      log(c.dim('  ') + metaParts.join(c.muted('  ·  ')));
    }
    ...
    log(c.dim('  ') + '─'.repeat(Math.min(40, (process.stdout.columns || 80) - 4)));
    log(c.code('  ' + displayContent));
    log('');
